...
async def ping(e):
    if e.sender_id in SUDO_USERS:
        if e.reply_to_msg_id:
            fuk = await e.respond("LE MADERCHOD HOGYA PING BHENNCHOD CHAIN HI NHI HAI BTW PAPA BOLTE TO. @THE_DEMON_NETWORK @MRSARTHAKZZ @AadixD @SMITMOREXD", reply_to=e.reply_to_msg_id)
        else:
            fuk = await e.reply("LE MADERCHOD HOGYA PING BHENNCHOD CHAIN HI NHI HAI BTW PAPA BOLTE TO. @THE_DEMON_NETWORK @MRSARTHAKZZ @AadixD @SMITMOREXD")
        start = datetime.now()
        end = datetime.now()
        ms = (end-start).microseconds / 1000
        pingop = f"LE MADERCHOD HOGYA PING BHENNCHOD CHAIN HI NHI HAI BTW PAPA BOLTE TO. @THE_DEMON_NETWORK @MRSARTHAKZZ @AadixD @SMITMOREXD "                   
